...
// ** pub, msg define & func
std::array<rclcpp::Publisher<ms_module_msgs::msg::JointCmdList>::SharedPtr, 5> limb_x_joint_cmd_list_pubs;
std::array<rclcpp::Subscription<sensor_msgs::msg::JointState>::SharedPtr, 5> limb_x_joint_state_list_subs;
...
int main(int argc, char * argv[])
{
    ...
    std::array<std::array<CModule_Joint_CMD, 4>, 5> limb_joint_cmd;
    for(int i=0; i<5; i++){
        for(int j=0; j<4; j++){
    ...
    // *** define publisher msgs
    std::array<ms_module_msgs::msg::JointCmdList, 5> cmds;
    for (int i=0; i<5; i++){
